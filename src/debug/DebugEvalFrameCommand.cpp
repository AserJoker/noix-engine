...
DebugEvalFrameCommand::DebugEvalFrameCommand(script::ScriptEngine& engine)
    : _engine(engine) {}

std::string DebugEvalFrameCommand::execute(const std::string& arguments) {
    ...
    _engine.postTask([&]() {
        ...
        if (agent && agent->isPaused()) {
            result = agent->evaluateInFrame(frameIndex, expr);
        } else {
