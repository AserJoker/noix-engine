...
bool Application::initWindow() {
  ...
  WindowMode mode = parseWindowMode(
      windowCfg.has("mode") ? windowCfg["mode"].asString() : "windowed");
  Resolution res = parseResolution(
      windowCfg.has("resolution") ? windowCfg["resolution"].asString() : "fhd");
  ...
  _window = SDL_CreateWindow(
      _localeManager->i18n("noix:system.window.title", "noix-engine").c_str(),
      width, height, flags);
  if (!_window) {
    core::Logger::instance().error("SDL_CreateWindow failed: {}",
                                   SDL_GetError());
    return false;
  }
  ...
  core::Logger::instance().info("Window created ({}x{}, {})", width, height,
                                toString(mode));
  return true;
}
...
void Application::initLogger() {
  ...
  rotateLogs(logsDir);
  logger.addSink(
      std::make_shared<core::FileSink>((logsDir / "engine.log").string()));
...
void Application::initResourcePack() {
  ...
  core::Logger::instance().info("AssetManager initialized (basePath={})",
                                _basePath);
...
void Application::initDebugServer() {
  uint32_t shutdownEventType = SDL_RegisterEvents(1);
  uint32_t freezeEventType = SDL_RegisterEvents(1);
  uint32_t resumeEventType = SDL_RegisterEvents(1);

  /* ScriptEngine — owns QuickJS runtime on its thread */
  _scriptEngine = std::make_unique<script::ScriptEngine>(_basePath);
  _scriptEngine->setDebugEventTypes(freezeEventType, resumeEventType);

  /* EventBus — event dispatch via SDL custom events */
  uint32_t eventBusEventType = SDL_RegisterEvents(1);
  _eventBus = std::make_unique<EventBus>();
  _eventBus->setEventType(eventBusEventType);
  ...
  registerEventHandler(freezeEventType,
                       [this](const SDL_Event &) { _frozen.store(true); });
  registerEventHandler(resumeEventType,
                       [this](const SDL_Event &) { _frozen.store(false); });
  registerEventHandler(eventBusEventType, [this](const SDL_Event &e) {
    _eventBus->handleSdlEvent(e);
  });

  /* DapServer — DAP protocol debug server (TCP only) */
  uint16_t dapPort = 0;
  if (_args.has("dap-port"))
    dapPort = static_cast<uint16_t>(std::stoi(_args.get("dap-port")));
  ...
  if (_args.has("debug-port"))
    httpPort = static_cast<uint16_t>(std::stoi(_args.get("debug-port")));
...
void Application::signalHandler(int) { requestShutdown(); }
...
int Application::run() {
  ...
  try {
...

    {
      ...
      _configManager->getOrDefault(core::NamespacedId("noix", "application"),
                                   defaults);
    }
...
void Application::registerEventHandler(uint32_t eventType,
                                       EventHandler handler) {
  _eventHandlers[eventType] = std::move(handler);
}
