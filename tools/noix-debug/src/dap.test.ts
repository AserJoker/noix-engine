...
class SafeDebugClient extends DebugClient {
	start(port: number): Promise<void> {
		return new Promise((resolve, reject) => {
			const socket = net.createConnection(port, '127.0.0.1', () => {
				socket.on('error', (err: any) => {
					if (err.code !== 'ECONNRESET' && err.code !== 'EPIPE') throw err;
				});
...
async function withSession(
	scriptPath: string,
	testFn: (dc: DebugClient, port: number) => Promise<void>,
): Promise<void> {
	const port = await findFreePort();
	const proc = await new Promise<ChildProcess>((resolve, reject) => {
		const p = spawn(BRIDGE_EXE, ['--port', String(port), '--script', scriptPath], {
			stdio: ['pipe', 'pipe', 'pipe'],
		});
		let started = false;
		p.stderr!.on('data', (data: Buffer) => {
			const msg = data.toString();
			if (msg.includes('[DAP]')) process.stderr.write('[BRIDGE] ' + msg);
			if (!started && msg.includes('listening on port')) {
				started = true;
				resolve(p);
			}
		});
		p.on('error', (err) => { if (!started) reject(err); });
		setTimeout(() => { if (!started) { p.kill(); reject(new Error('Bridge start timeout')); } }, 5000);
	});
	...
	try {
		...
		await new Promise<void>(r => { proc.on('exit', () => setTimeout(r, 200)); setTimeout(r, 2000); });
	}
}

/** Initialize + attach, return the stopped event body from debugger statement */
async function initAndAttach(dc: DebugClient, port: number): Promise<any> {
	const stoppedPromise = dc.waitForEvent('stopped');
	await dc.initializeRequest(INIT_ARGS);
	await dc.attachRequest({ port } as any);
...
describe('DAP Debug Bridge', function () {
	...
	it('should return local variables with correct values', async () => {
		await withSession(ADVANCED_SCRIPT, async (dc, port) => {
			...
			const xVar = allVars.find((v: any) => v.name === 'x');
			const yVar = allVars.find((v: any) => v.name === 'y');
			const zVar = allVars.find((v: any) => v.name === 'z');
	...
	it('should expand object variables', async () => {
		await withSession(ADVANCED_SCRIPT, async (dc, port) => {
			...
			const objVar = allVars.find((v: any) => v.name === 'obj');
			...
			const nameVar = objVarsResp.body.variables.find((v: any) => v.name === 'name');
			assert.ok(nameVar, 'obj.name should be present');
			assert.ok(nameVar.value === '"noix"' || nameVar.value === 'noix',
				`Expected "noix" or noix, got ${nameVar.value}`);
	...
	it('should expand array variables', async () => {
		await withSession(ADVANCED_SCRIPT, async (dc, port) => {
			...
			const arrVar = allVars.find((v: any) => v.name === 'arr');
	...
	it('should evaluate object with variablesReference', async () => {
		await withSession(ADVANCED_SCRIPT, async (dc, port) => {
			...
			const nameVar = objVars.body.variables.find((v: any) => v.name === 'name');
	...
	it('should stop on exception when "all" filter is set', async () => {
		await withSession(ADVANCED_SCRIPT, async (dc, port) => {
			...
			for (let i = 0; i < 10 && !done; i++) {
				...
				if ((result as any).event === 'terminated') done = true;
			}
		});
	});
	...
	it('should return thread list', async () => {
		await withSession(SCRIPT_PATH, async (dc, port) => {
			...
			const mainThread = resp.body.threads.find((t: any) => t.id === 1);
	...
	it('should return loaded sources', async () => {
		await withSession(SCRIPT_PATH, async (dc, port) => {
			await initAndAttach(dc, port);
			const resp = await (dc as any).send('loadedSources', {});
			assert.ok(resp.body.sources.length >= 1, 'Should have at least one source');
			const mainSource = resp.body.sources.find((s: any) =>
				s.path && s.path.includes('test_debug'));
	...
	it('should report correct stack depth inside nested function', async () => {
		await withSession(ADVANCED_SCRIPT, async (dc, port) => {
			...
			const stack = await dc.stackTraceRequest({ threadId: 1 });
			assert.ok(stack.body.stackFrames.length >= 2,
				`Expected >= 2 frames in nested call, got ${stack.body.stackFrames.length}`);
